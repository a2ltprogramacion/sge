...
-- 2. USUARIOS Y ROLES (Autenticación Global)
...
-- 3. DOCENTES (Especialización de Usuario)
...
-- 4. REPRESENTANTES (Especialización de Usuario)
...
-- 8. MATRÍCULAS (Relación Estudiante - Sección)
...
CREATE TABLE IF NOT EXISTS evaluaciones_items (
    ...
    fecha_aplicacion TEXT NOT NULL, -- Formato: 'YYYY-MM-DD'
    created_at TEXT NOT NULL DEFAULT (datetime('now'))
);

-- 12. CALIFICACIONES INDIVIDUALES
...
-- 14. REGISTRO ADMINISTRATIVO DE PAGOS
...
-- COBERTURA DE ÍNDICES CRÍTICOS (Prevenir table scans en consultas con agregación masiva)
